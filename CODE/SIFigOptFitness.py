...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  6 12:41:09 2021
...
pL = (1-x)*(R0*x+S0*(1-x))+x*(R1*x+S1*(1-x))
pH = (1-x)*(T0*x+P0*(1-x))+x*(T1*x+P1*(1-x))
...
# ax11.set_ylim((0,1))
# ax11.plot(tm,nm,color = ENVc,linewidth = envWidth,alpha=1,label='Env state')
# ax11.plot(tm,zLmm,color = LMc,linewidth = stratWidth,alpha=opac,label='freq of L-strategy, mypoic')
# ax11.plot(tm,zHmm,color = HMc,linewidth = stratWidth,alpha=opac,label='freq of H-strategy, mypoic')
...
# ax12.set_ylim((0,1))
# ax12.plot(tf,nf,color = ENVc,linewidth = envWidth,alpha=1,label='Env state')
# ax12.plot(tf,zLff,color = LFc,linewidth = stratWidth,alpha=opac,label='freq of L-strategy, forecaster')
# ax12.plot(tf,zHff,color = HFc,linewidth = stratWidth,alpha=opac,label='freq of H-strategy, forecaster')
...
# ax13.set_ylim((0,1))
# ax13.plot(t,n,color = ENVc,linewidth = envWidth,alpha=1,label='Env state')
# ax13.plot(t,zLm,color = LMc,linewidth = stratWidth,alpha=opac,label='freq of L-strategy, mypoic')
# ax13.plot(t,zHm,color = HMc,linewidth = stratWidth,alpha=opac,label='freq of H-strategy, mypoic')
# ax13.plot(t,zLf,color = LFc,linewidth = stratWidth,alpha=opac,label='freq of L-strategy, forecaster')
# ax13.plot(t,zHf,color = HFc,linewidth = stratWidth,alpha=opac,label='freq of H-strategy, forecaster')
...
ax21.plot([.4*tEndInv,insetT*tEndInv+.5*Shown*oneCycle,insetT*tEndInv+.5*Shown*oneCycle,.38*tEndInv],[.7,.3,.3,.95],'k-',linewidth=.75)
ax21.plot([insetT*tEndInv+.15*Shown*oneCycle,insetT*tEndInv+.15*Shown*oneCycle],[.02,.95],'k-',linewidth=2)
ax21.legend(loc='upper right')

inset = plt.axes([insetLoc, 0.77, .1, .1])
inset.set_xlim((insetT*tEndInv,insetT*tEndInv+.5*Shown*oneCycle))
inset.set_ylim((0,1))
...
ax22.plot((0,t[-1]),(fOpt,fOpt),linestyle = 'dashed',color ='k', label='Optimal long-run fitness')
...
yMax = np.max((fitFore[-1],invMax,fOpt)) + .2*(np.max((fitFore[-1],invMax)) - np.min(fitAvgM))
# az0.set_ylim((yMin,yMax))
ax22.set_ylim((yMin,yMax))
ax22.set_xlim((0,tEndInv-100))
...
if save=='yes' and final !='yes':
    ...
    fig2.savefig('../FIGS/SIFig2.png',dpi=300,bbox_extra_artists=(pars2,), bbox_inches='tight')
if save=='yes' and final =='yes':
    fig2.savefig('../FIGS/SIFig2.png',dpi=300, bbox_inches='tight')
